...
const INITIAL_FORM_STATE = {
  avatarUrl: "", // NEW: comes from Register via navigate state
...
export default function CreateMentorProfile() {
  // NEW: read avatarUrl that was passed from Register
  const location = useLocation();
  const avatarUrlFromRegister = location.state?.avatarUrl || "";

  const [role, setRole] = useState("mentor");
  const [form, setForm] = useState({
    ...INITIAL_FORM_STATE,
    avatarUrl: avatarUrlFromRegister,
  });
  ...
  function handleRoleSwitch(newRole) {
    ...
    // Keep avatar even when switching role
    setForm((prev) => ({
      ...INITIAL_FORM_STATE,
  ...
  function submit(e) {
    ...
    // avatarUrl will now be in the payload (frontend only)
  ...
  return (
    <div dir="rtl" style={styles.page}>
      ...
      <form onSubmit={submit} style={styles.card}>
        {/* NOTE: We are only carrying avatarUrl in state for now.
            If you want to show it on this page too, tell me and I'll add a small preview block. */}
            ...
            {role === "apprentice" && (
              <>
                ...
                <SelectField
                  ...
                  onChange={handleChange}
                  options={[{ v: "", t: "בחרי שנה" }, ...START_YEARS]}
                />
              </>
            )}
                ...
                <div style={styles.inline}>
                  {["א'", "ב'", "ג'", "ד'", "ה'", "ו'", "ז'"].map((y) => (
                    <button
                      ...
                      style={{
                        ...styles.pillBtn,
                        ...(form.yearOfStudy === y ? styles.pillBtnActive : {}),
                      }}
                    >
                      {y}
                    </button>
                ...
                <div style={styles.inline}>
                  {["כן", "לא"].map((opt) => (
                    <button
                      ...
                      style={{
                        ...styles.pillBtn,
                        ...(form.isShebaEmployee === opt ? styles.pillBtnActive : {}),
                      }}
                    >
                      {opt}
                    </button>
                ...
                <div style={styles.inline}>
                  {["כן", "לא"].map((opt) => (
                    <button
                      key={opt}
                      type="button"
                      onClick={() => updateField("hasMentoringExperience", opt)}
                      style={{
                        ...styles.pillBtn,
                        ...(form.hasMentoringExperience === opt ? styles.pillBtnActive : {}),
                      }}
                    >
                      {opt}
                    </button>
                ...
                <div style={styles.inline}>
                  {["כן", "לא"].map((opt) => (
                    <button
                      key={opt}
                      type="button"
                      onClick={() => updateField("hasResearchExperience", opt)}
                      style={{
                        ...styles.pillBtn,
                        ...(form.hasResearchExperience === opt ? styles.pillBtnActive : {}),
                      }}
                    >
                      {opt}
                    </button>
              ...
              <div className="mentor-grid" style={styles.grid}>
                ...
                <div style={styles.field}>
                  ...
                  <div style={styles.inline}>
                    {["כן", "לא"].map((opt) => (
                      <button
                        key={opt}
                        type="button"
                        onClick={() => updateField("isAvailableForResearch", opt)}
                        style={{
                          ...styles.pillBtn,
                          ...(form.isAvailableForResearch === opt ? styles.pillBtnActive : {}),
                        }}
                      >
                        {opt}
                      </button>
                  ...
                  {errors.isAvailableForResearch && (
                    <div style={styles.error}>{errors.isAvailableForResearch}</div>
                  )}
                </div>
              </div>
        ...
        <div style={{ ...styles.section, borderTop: "1px solid #eee", paddingTop: 24 }}>
          ...
          />
          <TextAreaField
            label="לקבלת חוות דעת ממנחים/מתלמדים"
            name="recommendationRequest"
            value={form.recommendationRequest}
            onChange={handleChange}
            placeholder="תואר + שם מלא + דואר אלקטרוני"
            rows={4}
          />
