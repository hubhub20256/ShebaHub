...
import React, { useEffect, useMemo, useState } from "react";
// וודאי שהשם כאן תואם לשם הקובץ האמיתי (MentorsCard עם M גדולה או mentorsCard)
...
export default function Mentors() {
  ...
  // פתרון שגיאה 1: הגדרת mockMentors בתוך useMemo למניעת שגיאות Reference
  ...
  useEffect(() => {
    ...
    async function load() {
      if (!showReal || realMentors.length > 0) return;
      ...
      try {
        const data = await profilesAPI.listMentors();
        const list = Array.isArray(data) ? data : data?.results || [];
        if (isMounted) setRealMentors(list);
      } catch (err) {
        if (isMounted) {
          // פתרון שגיאה 2: גישה בטוחה למידע על השגיאה (err)
          const errorMessage = err?.response?.data?.detail || err?.message || "לא הצלחתי לטעון מנחים";
    ...
    <div className="mentors-page" dir="rtl">
      ...
      <div className="mentors-controls-container">
        <div className="mode-switch-row">
          <button
            type="button"
            className={`mode-switch-btn ${showReal ? 'is-real' : 'is-mock'}`}
            onClick={() => setShowReal((v) => !v)}
          >
            {showReal ? "מציג מנחים מהמערכת" : "הצג מנחים אמיתיים (זמני)"}
          </button>
        </div>
        ...
        {realLoading && <p className="mentors-status-msg">טוען נתונים מהשרת...</p>}
        {realError && <p className="mentors-status-msg error">{realError}</p>}
      </div>

      {/* פתרון שגיאה 3: וודאי שכל כרטיס מקבל Key ייחודי (m.id || m.email) */}
      ...
      {filteredMentors.length === 0 && !realLoading && (
        <p className="mentors-no-results">לא נמצאו תוצאות לחיפוש הנוכחי.</p>
      )}
    </div>
  );
}