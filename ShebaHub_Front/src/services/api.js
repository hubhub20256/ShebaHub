...
export const profilesAPI = {
  ...
  listMentors: (params = {}) => {
    ...
    return apiRequest(`/profiles/mentors/${queryString ? '?' + queryString : ''}`);
  },
  ...
  listStudents: (params = {}) => {
    const queryString = new URLSearchParams(params).toString();
    return apiRequest(`/profiles/students/${queryString ? '?' + queryString : ''}`);
  },
...
export const researchAPI = {
  ...
  cancelMyApplication: (id) => apiRequest(`/research/${id}/cancel/`, { method: 'DELETE' }),
  leaveResearch: (id) => apiRequest(`/research/${id}/leave/`, { method: 'POST' }),
  ...
  listMyApplications: (statusFilter = 'all') =>
    apiRequest(`/research/my-applications/?status=${encodeURIComponent(statusFilter)}`),
  ...
  approveResearchApplication: (researchId, applicationId, note = '') =>
    apiRequest(`/research/me/${researchId}/applications/${applicationId}/approve/`, {
      method: 'POST',
      body: note ? { note } : undefined,
    }),
  rejectResearchApplication: (researchId, applicationId, note = '') =>
    apiRequest(`/research/me/${researchId}/applications/${applicationId}/reject/`, {
      method: 'POST',
      body: note ? { note } : undefined,
    }),
  removeResearchStudent: (researchId, applicationId, note = '') =>
    apiRequest(`/research/me/${researchId}/applications/${applicationId}/remove/`, {
      method: 'POST',
      body: note ? { note } : undefined,
    }),
  ...
  updateMentorPermissions: (researchId, applicationId, permissions) =>
    apiRequest(`/research/me/${researchId}/applications/${applicationId}/permissions/`, {
      method: 'PATCH',
      body: permissions,
    }),
  ...
  deleteChatMessage: (researchId, messageId) =>
    apiRequest(`/research/me/${researchId}/chat/${messageId}/`, { method: 'DELETE' }),
  togglePinMessage: (researchId, messageId) =>
    apiRequest(`/research/me/${researchId}/chat/${messageId}/pin/`, { method: 'POST' }),
...
export const referenceAPI = {
  ...
  getMedicalTrainingStages: () => apiRequest('/reference-data/medical-training-stages/'),
  getWorkTypes: () => apiRequest('/reference-data/work-types/'),
  getCompensationPreferences: () => apiRequest('/reference-data/compensation-preferences/'),
  getParticipationModes: () => apiRequest('/reference-data/participation-modes/'),
  getProfessionalExperience: () => apiRequest('/reference-data/professional-experience/'),
};
...
export const messagesAPI = {
  ...
  markRead: (id) => apiRequest(`/research/messages/${id}/read/`, { method: 'PATCH' }),
  markAllRead: () => apiRequest('/research/messages/mark-all-read/', { method: 'PATCH' }),
...
export const adminAPI = {
  ...
  listResearches: (params = {}) => {
    const queryString = new URLSearchParams(params).toString();
    return apiRequest(`/admin-panel/researches/${queryString ? '?' + queryString : ''}`);
  },
  approveResearch: (id, note = '') =>
    apiRequest(`/admin-panel/researches/${id}/approve/`, { method: 'POST', body: note ? { note } : undefined }),
  rejectResearch: (id, note = '') =>
    apiRequest(`/admin-panel/researches/${id}/reject/`, { method: 'POST', body: note ? { note } : undefined }),
  flagResearch: (id, note = '') =>
    apiRequest(`/admin-panel/researches/${id}/flag/`, { method: 'POST', body: note ? { note } : undefined }),
  softDeleteResearch: (id, note = '') =>
    apiRequest(`/admin-panel/researches/${id}/soft-delete/`, { method: 'POST', body: note ? { note } : undefined }),
  restoreResearch: (id) =>
    apiRequest(`/admin-panel/researches/${id}/restore/`, { method: 'POST' }),
  editResearch: (id, data) =>
    apiRequest(`/admin-panel/researches/${id}/edit/`, { method: 'PATCH', body: data }),
  bulkResearchAction: (ids, action, note = '') =>
    apiRequest('/admin-panel/researches/bulk/', { method: 'POST', body: { ids, action, note } }),
  exportResearchesCsv: (params = {}) => {
    const queryString = new URLSearchParams(params).toString();
    return downloadFile(`/admin-panel/researches/export/${queryString ? '?' + queryString : ''}`, 'researches.csv');
  },

  // User management
  listUsers: (params = {}) => {
    const queryString = new URLSearchParams(params).toString();
    return apiRequest(`/admin-panel/users/${queryString ? '?' + queryString : ''}`);
  },
  getUser: (id) => apiRequest(`/admin-panel/users/${id}/`),
  editUser: (id, data) =>
    apiRequest(`/admin-panel/users/${id}/edit/`, { method: 'PATCH', body: data }),
  deactivateUser: (id) => apiRequest(`/admin-panel/users/${id}/deactivate/`, { method: 'POST' }),
  reactivateUser: (id) => apiRequest(`/admin-panel/users/${id}/reactivate/`, { method: 'POST' }),
  forceVerifyUser: (id) => apiRequest(`/admin-panel/users/${id}/force-verify/`, { method: 'POST' }),
  bulkUserAction: (ids, action) =>
    apiRequest('/admin-panel/users/bulk/', { method: 'POST', body: { ids, action } }),
  exportUsersCsv: (params = {}) => {
    const queryString = new URLSearchParams(params).toString();
    return downloadFile(`/admin-panel/users/export/${queryString ? '?' + queryString : ''}`, 'users.csv');
  },

  // Audit logs
  getLogs: (params = {}) => {
    const queryString = new URLSearchParams(params).toString();
    return apiRequest(`/admin-panel/logs/${queryString ? '?' + queryString : ''}`);
  },
  exportLogsCsv: (params = {}) => {
    const queryString = new URLSearchParams(params).toString();
    return downloadFile(`/admin-panel/logs/export/${queryString ? '?' + queryString : ''}`, 'admin_logs.csv');
  },

  // Site settings
  getSettings: () => apiRequest('/admin-panel/settings/'),
  updateSettings: (data) => apiRequest('/admin-panel/settings/', { method: 'PATCH', body: data }),

  // Announcements
  listAnnouncements: () => apiRequest('/admin-panel/announcements/'),
  createAnnouncement: (data) =>
    apiRequest('/admin-panel/announcements/create/', { method: 'POST', body: data }),
  updateAnnouncement: (id, data) =>
    apiRequest(`/admin-panel/announcements/${id}/update/`, { method: 'PATCH', body: data }),
  deactivateAnnouncement: (id) =>
    apiRequest(`/admin-panel/announcements/${id}/deactivate/`, { method: 'POST' }),
  getActiveAnnouncements: () => apiRequest('/admin-panel/announcements/active/'),
  ...
  listApplications: (params = {}) => {
    const queryString = new URLSearchParams(params).toString();
    return apiRequest(`/admin-panel/applications/${queryString ? '?' + queryString : ''}`);
  },
  overrideApplicationStatus: (id, new_status, note = '') =>
    apiRequest(`/admin-panel/applications/${id}/override/`, { method: 'POST', body: { new_status, note: note || undefined } }),
};
